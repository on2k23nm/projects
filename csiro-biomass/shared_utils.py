...
def show_one_image_per_species(df, img_dir, fig_w=14, max_species=None, seed=42):
    """
    Shows 1 image per species (random sample_id), arranged 3 images per row.
    df must have columns: 'Species', 'sample_id'
    Loads: img_dir / f"{sample_id}.jpg"
    """
    ...
    # pick 1 id per species
    picks = []
    for sp in species_list:
        ids = df.loc[df["Species"] == sp, "sample_id"].dropna().astype(str).unique().tolist()
        rng.shuffle(ids)
        sid = ids[0] if ids else None
        picks.append((sp, sid))

    # estimate aspect ratio from first readable image
    aspect = 0.75  # fallback
    for sp, sid in picks:
        if sid is None:
            continue
        p = os.path.join(img_dir, f"{sid}.jpg")
        im = cv2.imread(p)
        if im is not None:
            h, w = im.shape[:2]
            aspect = h / max(w, 1)
    ...
    fig_h = fig_w * (R / C) * aspect
    fig, axes = plt.subplots(R, C, figsize=(fig_w, max(2.0, fig_h)), squeeze=False)

    for i, (sp, sid) in enumerate(picks):
        r, c = divmod(i, C)
        ax = axes[r][c]
        ...
        # simple default ruler (ticks)
        ax.set_title(f"{sid} | {sp}", fontsize=9)
...
def scatter_raw_and_log1p_one_row(df, cols, *, max_points=5000, log1p_eps=0.0,
                                 point_size=10, alpha=0.6, bins=40, show_titles=True):
    """
    Layout: 2 rows × (2 * N) columns

    Row 1: [RAW scatter(col1), LOG1P scatter(col1), RAW scatter(col2), LOG1P scatter(col2), ...]
    Row 2: [RAW hist(col1),    LOG1P hist(col1),    RAW hist(col2),    LOG1P hist(col2),    ...]
    """
    if isinstance(cols, str):
        cols = [cols]
    n = len(df)
    if n == 0 or len(cols) == 0:
        print("Empty df or cols.")
        return

    # sample rows (same rows for all columns)
    ...
    for j, col in enumerate(cols):
        ...
        # log1p guard (handles tiny negatives safely)
        y_safe = np.maximum(y, -1 + 1e-12) + log1p_eps
        y_log  = np.log1p(y_safe)

        # ---------- Row 1: SCATTER ----------
        ax_raw_sc = axes[0, 2*j]
        ax_raw_sc.scatter(x_idx, y, s=point_size, alpha=alpha)
        ax_raw_sc.set_xlabel("index")
        ax_raw_sc.set_ylabel(col)
        if show_titles:
            ax_raw_sc.set_title(f"RAW {col}")

        ax_log_sc = axes[0, 2*j + 1]
        ax_log_sc.scatter(x_idx, y_log, s=point_size, alpha=alpha)
        ax_log_sc.set_xlabel("index")
        ax_log_sc.set_ylabel(f"log1p({col})")
        if show_titles:
            ax_log_sc.set_title(f"LOG1P {col}")

        # ---------- Row 2: DISTRIBUTION ----------
        ax_raw_hist = axes[1, 2*j]
        ax_raw_hist.hist(y[~np.isnan(y)], bins=bins)
        ax_raw_hist.set_xlabel(col)
        ax_raw_hist.set_ylabel("count")
        if show_titles:
            ax_raw_hist.set_title(f"RAW dist {col}")

        ax_log_hist = axes[1, 2*j + 1]
        ax_log_hist.hist(y_log[~np.isnan(y_log)], bins=bins)
        ax_log_hist.set_xlabel(f"log1p({col})")
        ax_log_hist.set_ylabel("count")
        if show_titles:
            ax_log_hist.set_title(f"LOG1P dist {col}")
